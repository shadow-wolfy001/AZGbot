...
client.on('message', msg => {
  if (msg.content === '*help') {
    msg.reply('My current comands are *channel (shows you Aliens youtube)  *invites (shows the amount of invites you have), *report (to report a bug or a member), *serverinvite (shows the invite for this server incase you want to invite a friend), ban and kick (mods only) and the say command. more commands soon though :wink:')
  }
  });
...
client.on("message", async message => {
  ...
  if(command === "ban") {
    ...
    if(!message.member.roles.cache.some(r=>["Moderator"].includes(r.name)))
      return message.reply("Sorry, you don't have permissieons to use this!");
